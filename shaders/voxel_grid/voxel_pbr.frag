...
layout(location = 6) in vec3 frag_voxel_color;
layout(location = 7) in float frag_voxel_ao;
...
void main()
{
    ...
    vec3 albedo = clamp(material_data.color.rgb * frag_voxel_color, 0.0, 1.0);
    ...
    float ao = material_data.material.z > 0.0
        ...
        : 1.0;
    float softened_voxel_ao = mix(1.0, frag_voxel_ao, 0.45);
    ao = clamp(ao * softened_voxel_ao, 0.0, 1.0);
    ...
    vec3 terrain_ambient = albedo * vec3(0.18, 0.20, 0.22) * mix(0.65, 1.25, up) * ao;
    ambient = max(ambient, terrain_ambient);
