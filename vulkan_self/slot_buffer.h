...
class SlotBuffer {
public:
    _XCLASS_NAME(SlotBuffer);

    SlotBuffer(VulkanEngine& engine, uint32_t num_slots, uint32_t slot_size)
    ...
    static SlotBuffer create(VulkanEngine& engine, uint32_t num_slots) {
        static_assert(
            std::is_trivially_copyable_v<SlotType>,
            "Material buffer slot types should be trivially copyable"
        );

        return SlotBuffer(engine, num_slots, sizeof(SlotType));
    }
