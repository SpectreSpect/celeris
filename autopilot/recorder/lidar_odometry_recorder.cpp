#include "lidar_odometry_recorder.h"

LidarOdometryRecorder::~LidarOdometryRecorder() noexcept {
    stop();
}

void LidarOdometryRecorder::start(std::filesystem::path path) {
    LOG_METHOD();
...
void LidarOdometryRecorder::stop() {
    LOG_METHOD();
...
void LidarOdometryRecorder::record(LidarScan& lidar_scan, const Odometry& odometry) {
    LOG_METHOD();
    ...
    std::string scan_file_name = "scan_" + std::to_string(m_record_count) + ".lsb";
    std::filesystem::path scan_file_path = m_recording_dir / scan_file_name;

    lidar_scan.save(scan_file_path);
