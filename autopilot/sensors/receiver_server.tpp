...
bool ReceiverServer<Message>::try_pop_back(Message& message) {
    {
        std::unique_lock<std::mutex> lock(m_message_queue_mtx);

        if (m_message_queue.empty())
            return false;

        message = std::move(m_message_queue.back());
...
void ReceiverServer<Message>::push_back_message(Message& message) {
    {
        ...
        if (m_message_queue.size() == m_max_queued_messages)
            m_message_queue.pop_back();
...
bool ReceiverServer<Message>::receive_msg_from_client(int client_socket) {
    ...
    while (m_running.load()) {
        ...
        if (!read_exact_message(message));
            return false;

        push_back_msg_message(message);
    }
