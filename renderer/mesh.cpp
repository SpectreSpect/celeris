...
Mesh::Mesh(VulkanEngine& engine, VulkanResourceLoader& resource_loader, 
           void* vertex_data, uint32_t vertex_data_size_bytes, 
           unsigned int* index_data, uint32_t index_data_size_bytes) 
    :   Mesh(engine.physical_device(), engine.device(), resource_loader, vertex_data, vertex_data_size_bytes, index_data, index_data_size_bytes) {}
...
MeshView Mesh::get_view() {
    return MeshView(m_vertex_buffer.get_view(), m_index_buffer.get_view(), m_index_count);
}