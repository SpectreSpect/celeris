...
BrdfLutPass::BrdfLutPass(VulkanEngine& engine, ComputePassManager& compute_pass_manager)
    :   m_engine(engine),
        uniform_buffer(VulkanBuffer::create_host_visible_uniform_buffer(engine, sizeof(BrdfLutPass))),
        m_brdf_lut_pass(compute_pass_manager.descriptor_pool(), compute_pass_manager.brdf_lut_cp),
...
VulkanTexture2D BrdfLutPass::generate(uint32_t width, uint32_t height) {
    ...
    VulkanTexture2DDesc desc = VulkanTexture2D::deafult_desc({width, height});
    ...
    VulkanTexture2D brdf_lut(m_engine.physical_device(), m_engine.device(), desc);

    static BrdfLutGeneratorUniform uniform_data{};
    uniform_data.image_width = width;
    uniform_data.image_height = height;
    uniform_buffer.upload(&uniform_data, sizeof(BrdfLutGeneratorUniform));

    m_brdf_lut_pass.set_uniform_buffer(0, uniform_buffer);
...

    {
        ...
        m_brdf_lut_pass.set_storage_texture(1, brdf_lut);
        
        m_brdf_lut_pass.bind(m_compute_command_buffer);
