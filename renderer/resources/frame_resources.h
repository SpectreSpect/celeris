...
struct FrameData {
    ...
    VulkanBuffer lighting_build_uniform;       // compute only
    VulkanBuffer clustered_lighting_uniform;   // fragment only
    ...
    FrameData(
        ...
        VkDeviceSize lighting_build_uniform_size,
        VkDeviceSize clustered_lighting_uniform_size
    )
        ...
        lighting_build_uniform(
            VulkanBuffer::create_host_visible_uniform_buffer(
                engine,
                lighting_build_uniform_size
            )
        ),
        clustered_lighting_uniform(
            VulkanBuffer::create_host_visible_uniform_buffer(
                engine,
                clustered_lighting_uniform_size
            )
        )
    {}
};
...
public:
    ...
    void update_camera(uint32_t frame_id, const Window& window, const Camera& camera);
