...
OldGICPPass::OldGICPPass(VulkanEngine& engine, ComputePassManager& compute_pass_manager)
    :   engine(engine),
        gicp_step_pass(compute_pass_manager.old_gicp_cp, compute_pass_manager.descriptor_pool()),
        ...
        partial_src(VulkanBuffer::create_host_visible_storage_buffer(engine, sizeof(OldGICPReductor::GICPPartial) * max_partial_count)),
        partial_dst(VulkanBuffer::create_host_visible_storage_buffer(engine, sizeof(OldGICPReductor::GICPPartial) * max_partial_count)),
    ...
    // partial_src.create(engine, sizeof(OldGICPReductor::GICPPartial) * max_partial_count);
    // partial_dst.create(engine, sizeof(OldGICPReductor::GICPPartial) * max_partial_count);
    ...
    // reductor = OldGICPReductor(engine);
}

double OldGICPPass::step(OldVoxelPointMap& voxel_point_map, OldPointCloud& source_point_cloud, VulkanBuffer& source_normal_buffer) {
    LOG_METHOD();
    ...
    // std::vector<OldGICPReductor::GICPPartial> test_parcial_src;
    // test_parcial_src.resize(100);
    // for (int i = 0; i < test_parcial_src.size(); i++) {
    //     OldGICPReductor::GICPPartial parital{};
    ...
    // }
    // // partial_src.upload(test_parcial_src.data(), test_parcial_src.size() * sizeof(OldGICPReductor::GICPPartial), 0);
    ...
    OldGICPReductor::GICPPartial result = reductor.reduce(partial_src, partial_dst, partial_count);
...
double OldGICPPass::fit(OldVoxelPointMap& voxel_point_map, OldPointCloud& source_point_cloud, VulkanBuffer& source_normal_buffer, uint32_t max_steps) {
    double rmse = 0;
...
glm::quat OldGICPPass::omega_to_quat(const glm::vec3& omega) {
...
bool OldGICPPass::solve_6x6(const double H_in[6][6], const double g_in[6], double delta_out[6]) {
    // int counter = 0;
...
glm::mat3 OldGICPPass::omega_to_mat3(const glm::vec3& omega) {
    float theta = glm::length(omega);
