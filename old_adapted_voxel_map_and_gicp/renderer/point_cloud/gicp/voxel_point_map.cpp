...
#include "../../../../renderer/instance_buffer_view.h"
#include "../../../../voxel_grid_vulkan/voxel_grid_structures.h"
#include "../../../../voxel_grid_vulkan/voxel_grid.h"
#include "../../../../vulkan_self/utils.h"

OldVoxelPointMap::OldVoxelPointMap(VulkanEngine& engine, uint32_t num_hash_table_slots, uint32_t max_map_point_count)
    :   m_num_hash_table_slots(num_hash_table_slots),
        ...
        // map_point_buffer(VulkanBuffer::create_storage_buffer(engine, sizeof(OldPointInstance) * max_map_point_count)),
        map_point_buffer(VulkanBuffer(engine.physical_device(), engine.device(), sizeof(OldPointInstance) * max_map_point_count, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)),        
...
InstanceBufferView OldVoxelPointMap::get_map_instance_view() {
    return InstanceBufferView(map_point_buffer, m_map_point_count, sizeof(OldPointInstance));
}

void OldVoxelPointMap::upload_voxels(VulkanEngine& engine, VoxelGrid& voxel_grid) {
    std::vector<OldPointInstance> points(m_map_point_count);

    map_point_buffer.read(points.data(), sizeof(OldPointInstance) * m_map_point_count, 0);
