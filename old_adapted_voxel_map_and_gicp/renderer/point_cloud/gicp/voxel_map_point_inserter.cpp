...
OldVoxelMapPointInserter::OldVoxelMapPointInserter(VulkanEngine& engine, ComputePassManager& compute_pass_manager) 
    :   engine(engine),
        insert_pass(compute_pass_manager.old_point_voxel_map_insert_cp, compute_pass_manager.descriptor_pool()),
...
void OldVoxelMapPointInserter::insert(OldVoxelPointMap& voxel_point_map, OldPointCloud& source_point_cloud, VulkanBuffer& source_normal_buffer) {
    LOG_METHOD();
    // if (!this->engine)
    ...
    // source_point_cloud.instance_buffer()->upload(points.data(), points.size() * sizeof(OldPointInstance));
