...
OldLidarVideo::OldLidarVideo(ManagerBundle& manager_bundle, const std::filesystem::path& csv_path, int first_frame, int last_frame){
    load_from_file(manager_bundle, csv_path, first_frame, last_frame);
}

void OldLidarVideo::load_from_file(ManagerBundle& manager_bundle, const std::filesystem::path& csv_path, int first_frame, int last_frame) {
    ...
    for (const auto& e : entries) {
        // OldLidarScan* lidar_scan = new OldLidarScan(manager_bundle, video_dir_path / e.filename);
        ...
        glm::vec3 car_pos_eng = OldLidarScan::ros_pos_to_engine(e.position);
...
bool OldLidarVideo::parse_u64(const std::string& s, uint64_t& out) {
    try { out = std::stoull(s); return true; } catch (...) { return false; }
}

bool OldLidarVideo::parse_u32(const std::string& s, uint32_t& out) {
    try { out = static_cast<uint32_t>(std::stoul(s)); return true; } catch (...) { return false; }
}

bool OldLidarVideo::parse_f(const std::string& s, float& out) {
    try { out = std::stof(s); return true; } catch (...) { return false; }
}

void OldLidarVideo::mat4_to_pose(const glm::mat4& M, glm::vec3& position, glm::vec3& rotation) {
    position = glm::vec3(M[3]);
    rotation = mat3_to_euler_xyz_custom(glm::mat3(M));
}

glm::vec3 OldLidarVideo::ros_vec_to_engine(const glm::vec3& v_ros) {
    return basis_M_ros_to_engine() * v_ros;
}

glm::vec3 OldLidarVideo::mat3_to_euler_xyz_custom(const glm::mat3& R) {
...
glm::mat4 OldLidarVideo::pose_to_mat4(const glm::vec3& position, const glm::vec3& rotation) {
...
glm::vec3 OldLidarVideo::mat3_to_rpy_zyx(const glm::mat3& R) {
...
glm::mat3 OldLidarVideo::rpy_to_mat3(float roll, float pitch, float yaw) {
...
glm::mat3 OldLidarVideo::basis_M_ros_to_engine() {
...
glm::vec3 OldLidarVideo::ros_rpy_to_engine_rpy(const glm::vec3& rpy_ros) {
...
uint32_t OldLidarVideo::current_frame_id() const {
    return m_current_frame_id;
}

void OldLidarVideo::next_frame() {
    LOG_METHOD();
...
void OldLidarVideo::previous_frame() {
    LOG_METHOD();
...
void OldLidarVideo::set_frame(uint32_t id) {
    LOG_METHOD();
...
OldLidarScan& OldLidarVideo::get_scan(uint32_t scan_id) {
    LOG_METHOD();
...
uint32_t OldLidarVideo::get_scan_count() {
    return m_scans.size();
}

void OldLidarVideo::sync() {
    LOG_METHOD();
...
float OldLidarVideo::frame_time_seconds(size_t frame_id) const {
...
size_t OldLidarVideo::get_frame_id(float time, size_t search_start_id) const {
...
void OldLidarVideo::move(float time) {
    if (m_scans.empty())
        return;
...
void OldLidarVideo::pause() {
    m_paused = true;
}

void OldLidarVideo::resume() {
    m_paused = false;
}

void OldLidarVideo::set_looped(bool looped) {
    m_looped = looped;
}

bool OldLidarVideo::is_paused() const {
    return m_paused;
}

void OldLidarVideo::update(float delta_time) {
    if (m_paused)
        return;
