...
void OldLidarScan::set_timestamp_ns(uint64_t timestamp_ns) {
    m_timestamp_ns = timestamp_ns;
}

uint64_t OldLidarScan::timestamp_ns() const noexcept {
    return m_timestamp_ns;
}

OldLidarScan::FrameData OldLidarScan::read_frame_from_file(const std::filesystem::path& path) {
...
OldPointCloud OldLidarScan::load_from_file(ManagerBundle& manager_bundle, const std::filesystem::path& path) {
    return load_from_frame(manager_bundle, read_frame_from_file(path));
}

OldPointCloud OldLidarScan::load_from_frame(ManagerBundle& manager_bundle, FrameData&& frame) {
    m_timestamp_ns = frame.timestamp_ns;

    if (frame.points.empty()) {
        throw std::runtime_error("OldLidarScan frame had no points");
    }

    m_points = std::move(frame.points);

    return OldPointCloud(manager_bundle, m_points);
}

void OldLidarScan::build_points_for_frame(FrameData& frame) {
...
#if 0
std::vector<glm::vec4> OldLidarScan::calculate_normals(std::vector<OldPointInstance> points) {
...
glm::mat3 OldLidarScan::rpy_to_mat3_zyx(float roll, float pitch, float yaw)
    {
...
glm::vec3 OldLidarScan::ros_pos_to_engine(const glm::vec3& p_ros)
{
    return glm::vec3(-p_ros.x, p_ros.z, p_ros.y); // (-x, z, y)
}
    
OldPointCloud& OldLidarScan::point_cloud() {
    return m_point_cloud;
}

VulkanBuffer& OldLidarScan::normal_buffer() {
    return m_normal_buffer;
}

#if 0
void OldLidarScan::remove_points_near_origin(std::vector<OldPointInstance>& points,
                                           std::vector<glm::vec4>& normals,
                                           float min_distance)
{
    ...
    std::vector<OldPointInstance> filtered_points;
...
void OldLidarScan::drop_out_points_and_normals(std::vector<OldPointInstance>& points,
                                             std::vector<glm::vec4>& normals,
                                             size_t target_size)
{
    if (points.size() != normals.size()) {
    ...
    std::vector<OldPointInstance> new_points;
...
void OldLidarScan::keep_only_upward_facing_points_and_normals(
    ...
    float up_dot_threshold)
{
    ...
    std::vector<OldPointInstance> kept_points;
...
void OldLidarScan::remove_ground_points_and_normals(
    std::vector<OldPointInstance>& points,
    std::vector<glm::vec4>& normals,
    float up_dot_threshold,
...
void OldLidarScan::remove_invalid_points_and_normals(std::vector<OldPointInstance>& points,
                                                   std::vector<glm::vec4>& normals)
{
    if (points.size() != normals.size()) {
...
// void OldLidarScan::get_normals(std::vector<OldPointInstance> points, std::vector<glm::vec3> normals) {
//     int rings_count = 16;
...
// void OldLidarScan::draw(RenderState state) {
...
int OldLidarScan::xy_id(int x, int y, int ring_width, int cloud_size){
...
bool OldLidarScan::is_same_object(const OldPointInstance &p0, const OldPointInstance &p1,
...
void OldLidarScan::get_normals(const std::vector<OldPointInstance>& points, std::vector<glm::vec4>& normals)
{
    normals.clear();
    ...
    {
        const OldPointInstance& a = points[ia];
        const OldPointInstance& b = points[ib];
        const OldPointInstance& c = points[ic];
    ...
    for (int y = 0; y < rings_count - 1; y++) {
        for (int x = 0; x < ring_width - 1; x++) {
            ...
            const OldPointInstance& p0 = points[id1]; // lower-left
            const OldPointInstance& p1 = points[id2]; // upper-left
            const OldPointInstance& p2 = points[id3]; // upper-right
            const OldPointInstance& p3 = points[id5]; // lower-right
