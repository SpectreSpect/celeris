...
public:
    ...
    static std::vector<NonholonomicPos> discretize_path(
        NonholonomicPos start_pos,
        std::vector<NonholonomicPathElement> path,
        int points_per_circlar_element,
        float min_radius = 1.0f,
        std::optional<NonholonomicPos> end_pos = std::nullopt,
        float linear_step_world = 0.0f
    );
    ...
    static std::vector<NonholonomicPos> get_optimal_path_discretized(
